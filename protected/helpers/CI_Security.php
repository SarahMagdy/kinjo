...
class CI_Security {
	 ...
	 */
	 // protected
	public static $_never_allowed_str =	array(
	 ...
	 */
	 // protected
	 public static $_never_allowed_regex = array(
		'javascript\s*:',
	...
	public static function xss_clean($str, $is_image = FALSE)
	{
		...
		if (is_array($str))
		{
			...
			{
				$str[$key] = CI_Security::xss_clean($str[$key]);
			}
			return $str;
		}
		// Remove Invisible Characters
		$str = CI_Security::remove_invisible_characters($str);
		/*
		 * URL Decode
		 *
	...
	public function sanitize_filename($str, $relative_path = FALSE)
	{
		...
		$str = CI_Security::remove_invisible_characters($str, FALSE);
		do
		{
			$old = $str;
	...
	static function ChkString($unSafeStr)
	{
		$safeStr = '';
		
		$targets = array('SELECT', 'UPDATE' , 'DELETE' , 'TRUNCATE' , 'INSERT' , 'FROM' , 'JOIN' , 'LEFT' , 'RIGHT' , 'WHERE' , 'VALUES');
		foreach($targets as $t)
		{
		   if (stripos($unSafeStr , $t) !== false) {
				$unSafeStr = str_ireplace($t , '', $unSafeStr);
		    }
		}
	...
	static function ChkPost($myPost){
		foreach ($myPost as $key => $val) {
			...
			if(is_array($val)){
				foreach ($val as $key2 => $val2) {
					...
					if(is_array($val2)){
						foreach ($val2 as $key3 => $val3) {
							$Str3 =  CI_Security::xss_clean($val3);
							$cleaned3 = CI_Security::ChkString($Str3);
						...
						if($key2 != 'Token' && $key2 != 'password' && $key2 !='confirmpassword' && $key2 != 'pass'){
							$Str2 =  CI_Security::xss_clean($val2);
							$cleaned2 = CI_Security::ChkString($Str2);
				...
				if($key != 'Token' && $key != 'password' && $key !='confirmpassword' && $key != 'pass'){
					$Str =  CI_Security::xss_clean($val);
					$cleaned = CI_Security::ChkString($Str);
					
					$myPost[$key] = $cleaned;
					// echo '<pre/>';
					// echo $key;
				}
				// echo $key.'&nbsp&nbsp'.$cleaned;
				// echo $key.' : '.$cleaned.'       ';
