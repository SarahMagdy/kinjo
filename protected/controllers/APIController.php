...
class APIController extends Controller {
	...
	public function actionGetTypes()
	{
		...
		if($BuAcc > 0){	
			$WhrAcc = " WHERE type_id IN ( SELECT DISTINCT(type) as type
						FROM business_unit WHERE accid = ".$BuAcc.")";
		}
		...
		if ($Lang != 0 && $Lang != 2) {
			...
			$SQL = " SELECT type_id TypeID ,type_img AS TypeImg,
					 ...
					 FROM types 
					 LEFT JOIN types_lang ON type_lang_type_id = type_id AND type_lang_lang_id = ".$Lang.$WhrAcc;
		}else{
			$SQL = " SELECT type_id TypeID ,type_name AS TypeName,type_img AS TypeImg FROM types ".$WhrAcc;
	...
	public function actionAddToOrder() {
		...
		if (isset($_POST['order'])) {
			...
			$OrdType = 'cust';
			if(isset($JsonArr['ord_type'])){
				$OrdType = $JsonArr['ord_type'];
			}
			if($OrdType == 'cust'){
				$ResArr = CustLib::actionAddToOrder($JsonArr);
			}
			if($OrdType == 'wait'){
				$ResArr = CustLib::actionAddToOrderCp($JsonArr);
			}
		} else {
	...
	public function actionEditOrder() {
		...
		if (isset($_POST['order'])) {
			...
			if($OrdType == 'wait'){
				$ResArr = CustLib::actionEditOrderCp($JsonArr);
			}
			//$ResArr = CustLib::actionEditOrder($JsonArr);
	...
	public function actionRemoveFromOrder() {
		...
		if (isset($_POST['order'])) {
			...
			if($OrdType == 'wait'){
				$ResArr = CustLib::actionRemoveFromOrderCp($JsonArr);
			}
			//$ResArr = CustLib::actionRemoveFromOrder($JsonArr);
	...
	public function actionLoginCpanel() {
		...
		if (isset($_POST['cpanel'])) {
			...
			$CpJson = json_decode($CpArr,TRUE);
			
			$ResArr = CustLib::actionLoginCpanel($CpJson);
	...
	public function actionLogoutCpanel() {
		...
		if (isset($_POST['cpanel'])) {
			...
			$ResArr = CustLib::actionLogoutCpanel($CpJson);
	...
	public function actionBuTables()
	{
		header('Content-Type: application/json');
		...
		$ResArr = CustLib::actionBuTables($Arr);
...
//var_dump($db_img);
	...
	{		
		// $pram = 'confirm={"ReservationID":"19","cid":2}';
		$pram = 'cust={"cid":"2","res_type":"TA"}';
		$ch = curl_init();//http://192.168.1.3/kinjo/public/index.php/APIApp/DBSignIn

		// $pram = 'order={"id":"","cust_id":"1","AppSource":"0","p_id":"16","bu_id":"3","Q_Conf":[{"qnt":"1","conf":"3,8","color":"7"}]}';
		// $pram = 'cpanel={"DevID":"a6d91f0bfe35a86e","AppSource":"0","UserName":"waiter","long":"29.97364","lat":"31.243757","Password":"123"}';
		$ch = curl_init();
		// curl_setopt($ch, CURLOPT_URL, "http://kinjo-app.com/index.php/API/LoginCpanel");
		// curl_setopt($ch, CURLOPT_URL, "mykinjo/index.php/API/ConfirmReserve");
		curl_setopt($ch, CURLOPT_URL, "mykinjo/index.php/API/GetCustRese");
	...
	public function actionCancelReserve()
	{
		...
		if (isset($_POST['cancel'])) {
			...
			$reserveArr = $_POST['cancel'];

			$JsonArr = json_decode($reserveArr,TRUE);

			$ResArr = CustLib::actionCancelReserve($JsonArr);
	...
	public function actionConfirmReserve()
	{
		header('Content-Type: application/json');
		...
		if (isset($_POST['confirm'])) {
			...
			$reserveArr = $_POST['confirm'];

			$JsonArr = json_decode($reserveArr,TRUE);

			$ResArr = CustLib::actionConfirmReserve($JsonArr);
	...
	public function actionGetAllUnits()
	{
		...
		if (isset($_POST['unit'])) {
			...
			$ResArr = CustLib::actionGetAllUnits($JsonArr); 
	...
	public function actionGetCustRese()
	{
		header('Content-Type: application/json');
		...
		if (isset($_POST['cust'])) {
			...
			$ResArr = CustLib::actionGetCustRese($JsonArr); 
